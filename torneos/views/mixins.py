...
class TorneoViewMixin(View):
    ...
    def get_torneo(self):
        ...
        if not hasattr(self, "_comp"):
            try:
                t_id = self.kwargs['torneo_id']
                self._comp = get_object_or_404(Torneo, t_id=t_id)
            except KeyError:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.kwargs = kwargs
        self.get_torneo()  # Throws Http404 if not found
        parent = super(TorneoViewMixin, self)
...
class UserRegisteredMixin(TorneoViewMixin, LoggedInMixin):
...
class RequireOpenMixin(CheckAllowedMixin):
    ...
    def check_if_allowed(self, request):
        ...
        # return self.get_torneo().is_open
        return True
